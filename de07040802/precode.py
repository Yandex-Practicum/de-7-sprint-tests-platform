...
for _ids in tuple((n1, n2) for n1 in (40000,) for n2 in range(40001, 40001 + 221)):
    _message_id = _message_id + 1
    _direct_messages.append(
        _create_row(
            message=f'Message {_message_id} from user {_ids[0]} to user {_ids[1]}...',
            message_from=_ids[0],
            message_to=_ids[1],
...
_result = _posts + _reactions + _direct_messages
